...
// TODO: Move this to Silent Lib? Just copied it from Silent Gear...
public class DataResourceManager<T> implements ResourceManagerReloadListener, Iterable<T> {
    ...
    public boolean isResourceEnabled(T value) {
        return true;
    }
            ...
            for (ResourceLocation id : resources.keySet()) {
                ...
                if (resourceOptional.isPresent()) {
                    ...
                    if (json == null) {
                        ...
                        if (isResourceEnabled(value)) {
                            tryAddObject(name, value);
                        } else {
                            this.logger.info("Skipping {} \"{}\" as it is disabled in the config", this.typeName, name);
                        }
                    }
                }
            }
